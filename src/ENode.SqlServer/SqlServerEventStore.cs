...
namespace ENode.SqlServer
{
    public class SqlServerEventStore : IEventStore
    {
        ...
        private const string QueryEventsSql = "SELECT * FROM {0} with(nolock) WHERE AggregateRootId = @AggregateRootId AND Version >= @MinVersion AND Version <= @MaxVersion ORDER BY Version ASC";
