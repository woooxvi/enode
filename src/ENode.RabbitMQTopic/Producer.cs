...
using IRabbitMQChannel = RabbitMQ.Client.IChannel;
...
namespace RabbitMQTopic
{
    ...
    public class Producer
    {
        ...
        private readonly Dictionary<string, (int queueCount, string[] consumerGroups)> _topics = new Dictionary<string, (int, string[])>();
        ...
        public Producer RegisterTopic(string topic, int queueCount, string[] consumerGroups = null)
        {
            ...
            if (!_topics.ContainsKey(topic))
            {
                ...
                {
                    _topics.Add(topic, (queueCount, new string[] { string.Empty }));
                }
                else
                {
                    _topics.Add(topic, (queueCount, consumerGroups));
                }
            }
        ...
        public void Start()
        {
            if (Interlocked.CompareExchange(ref _isRunning, 1, 0) == 0)
            {
                ...
                if (_autoConfig)
                {
                    ...
                    {
                        var queueCount = _topics[topic].queueCount;
                        var consumerGroups = _topics[topic].consumerGroups;
                        ...
                        using (var channelForConfig = _amqpConnection.CreateChannelAsync().Result)
                        {
                            ...
                            //{
                            //    channelForConfig.ExchangeDeclareAsync($"{topic}-delayed", "x-delayed-message", true, false,
                            //        new Dictionary<string, object>
                            //        {
                            //            {"x-delayed-type", ExchangeType.Topic}
                            //        }).Wait();
                            //    channelForConfig.ExchangeBindAsync(topic, $"{topic}-delayed", "", null).Wait();
                            //}
        ...
        public SendResult SendMessage(Message message, string routingKey)
        {
            ...
            try
            {
                var queueCount = _topics[message.Topic].queueCount;
                ...
                else
                {
                    ...
                    if (item == null)
                    {
                        item = new RabbitMQChannelWithActiveTime(_amqpConnection.CreateChannelAsync().Result);
                        //item.Channel.TxSelectAsync();
                    }
                }
                ...
                try
                {
                    ...
                    item.Channel.BasicPublishAsync(
                ...
                {
                    return new SendResult(SendStatus.Failed, null, $"Wait for confirms failed({ex.Message}).");
                }
                ...
                if (_channelPool.TryPeek(out RabbitMQChannelWithActiveTime item) && _isRunning == 1)
                {
                    if (item.ActiveTime.Add(_maxIdleDuration) < DateTime.Now)
                    {
                        if (_channelPool.TryDequeue(out RabbitMQChannelWithActiveTime removedItem))
                        {
                            ...
                            {
                                removedItem.Channel.CloseAsync().Wait();
                            }
                        }
                    }
                }
