...
namespace ENode.RabbitMQ
{
    ...
    public class DomainEventPublisher : IMessagePublisher<DomainEventStreamMessage>
    {
        ...
        /// <returns></returns>
        public DomainEventPublisher InitializeRabbitMQ(ProducerSettings settings, int queueCount = 4, bool autoConfig = true)
        {
            InitializeENode();
            _producer = new Producer(settings, false, autoConfig);
            _eventTopicProvider.GetAllTopics().ForEach(topic => _producer.RegisterTopic(topic, queueCount));
        ...
        public Task PublishAsync(DomainEventStreamMessage eventStream)
        {
            var message = CreateTopicMessage(eventStream);
             _sendMessageService.SendMessageAsync(_producer, "events", string.Join(",", eventStream.Events.Select(x => x.GetType().Name)), message, eventStream.AggregateRootId, eventStream.Id, eventStream.Items);
