...
namespace ENode.RabbitMQ
{
    ...
    public class CommandConsumer
    {
        ...
        class CommandExecuteContext : ICommandExecuteContext
        {
            ...
            public string QueueMessageID => new Guid().ToString();
            ...
            {
                var act = ObjectContainer.Resolve<ActivitySource>()?.StartActivity("CommandExecuteContext.OnCommandExecutedAsync");
                if (!string.IsNullOrEmpty(_commandMessage.SagaId))
                {
                    ...
                    act?.SetTag("SagaId", _commandMessage.SagaId);
                }
                else
                {
                    _messageContext.Ack();
            ...
            public async Task<T> GetAsync<T>(object id, bool firstFromCache = true) where T : class, IAggregateRoot
            {
                ...
                var act = ObjectContainer.Resolve<ActivitySource>()?.StartActivity("CommandExecuteContext.GetAsync");
                if (firstFromCache)
                {
                ...
                act?.SetTag("AggregateRootExists", aggregateRoot != null);
                if (aggregateRoot != null)
                {
                    act?.SetTag("AggregateRootID", aggregateRoot?.UniqueId);
