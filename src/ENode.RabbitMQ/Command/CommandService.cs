...
namespace ENode.RabbitMQ
{
    ...
    public class CommandService : ICommandService
    {
        ...
        /// <returns></returns>
        public CommandService InitializeRabbitMQ(ProducerSettings settings, int queueCount, bool delayedCommandEnabled = false, bool autoConfig = true, CommandResultProcessor commandResultProcessor = null, string sagaIdCommandItemKey = "SagaId")
        {
            InitializeENode();
            _producer = new Producer(settings, delayedCommandEnabled, autoConfig);
            _commandTopicProvider.GetAllTopics().ForEach(topic => _producer.RegisterTopic(topic, queueCount));
        ...
        {
            _sendMessageService.SendMessageAsync(_producer, "command", command.GetType().Name, BuildCommandMessage(command, false, _sagaIdCommandItemKey), command.AggregateRootId, command.Id, command.Items);
        ...
        public Task<CommandResult> ExecuteAsync(ICommand command, CommandReturnType commandReturnType)
        {
            ...
            try
            {
                _sendMessageService.SendMessageAsync(_producer, "command", command.GetType().Name, BuildCommandMessage(command, true, _sagaIdCommandItemKey), command.AggregateRootId, command.Id, command.Items);
            }
            catch
            {
