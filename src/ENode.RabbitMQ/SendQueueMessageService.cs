...
namespace ENode.RabbitMQ
{
    internal class SendQueueMessageService
    {
        ...
        public void SendMessageAsync(Producer producer, string messageCategory, string messageTypeName, Message message, string routingKey, string messageId, IDictionary<string, string> messageExtensionItems)
        {
            try
            {
                var sendResult = producer.SendMessageAsync(message, routingKey).Result;
                ...
                {
                    throw new ECommon.IO.IOException($"Send message fail: {sendResult.ErrorMessage}");
                }
            }
            catch (Exception ex)
            {
                ...
                throw new ECommon.IO.IOException("Send message has exception.", ex);
            }
        }
    }
}
