...
namespace ENode.Commanding
{
    public class ProcessingCommandMailbox
    {
        ...
        public ProcessingCommandMailbox(string aggregateRootId, IProcessingCommandHandler messageHandler, IJsonSerializer jsonSerializer, ILogger logger, ActivitySource activitySource)
        {
        ...
        public async Task CompleteMessage(ProcessingCommand message, CommandResult result)
        {
            try
            {
                if (_messageDict.TryRemove(message.Sequence, out ProcessingCommand removed))
                {
                    ...
                    LastActiveTime = DateTime.Now;
                    _logger.InfoFormat("stop trace:{0}", message.Message.GetActivity()?.TraceId);
                    await message.CompleteAsync(result).ConfigureAwait(false);
                    if (result.Status == CommandStatus.Failed)
                    {
                        message.Message.GetActivity()?.SetTag("error", result.Result);
                    }
                }
            }
            catch (Exception ex)
            {
                message.Message.GetActivity()?.SetTag("error", ex.ToString());
            ...
            {
                message.Message.GetActivity()?.Stop();
            }
        }
            ...
            using (await _asyncLock.LockAsync().ConfigureAwait(false))
            {
                ...
                try
                {
                    ...
                    while (TotalUnHandledMessageCount > 0 && scannedCount < _batchSize && !IsPauseRequested)
                    {
                        ...
                        if (message != null)
                        {
                            var parentAct = message.Message?.GetActivity();
                            traceid = parentAct?.TraceId.ToString();
                            Activity act = null;
                            act = parentAct == null ? act = _activitySource?.StartActivity("ProcessMessages", ActivityKind.Internal) : _activitySource?.StartActivity("ProcessMessages", ActivityKind.Internal, parentAct.Context);
                            if (_duplicateCommandIdDict.ContainsKey(message.Message.Id))
                            {
                                message.IsDuplicated = true;
                                act?.AddTag("IsDuplicated", true);
                            }
                            await _messageHandler.HandleAsync(message).ConfigureAwait(false);
                            act?.Stop();
                            if (act == null) _logger.Error($"ProcessingCommandMailbox has null activity command. aggregateRootId:{AggregateRootId},cmd:{message.Message?.GetType().Name}");
                        }
                        scannedCount++;
                        ConsumingSequence++;
                    }
                ...
                {
                    _logger.Error(string.Format("{0} run has unknown exception, aggregateRootId: {1}, traceId: {2}", GetType().Name, AggregateRootId, traceid), ex);
                    Thread.Sleep(1);
                }
                finally
                {
