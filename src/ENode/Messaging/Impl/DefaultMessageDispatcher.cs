...
namespace ENode.Messaging.Impl
{
    public class DefaultMessageDispatcher : IMessageDispatcher
    {
        ...
        {
            var message = singleMessageDispatching.Message;
            var domainEvent = message as Eventing.IDomainEvent;
            _ioHelper.TryAsyncActionRecursivelyWithoutResult("HandleSingleMessageAsync",
          async () =>
          {
              try
              {
                  message.GetActivity()?.Start();
                  await handlerProxy.HandleAsync(message);
              }
              catch (Exception ex)
              {
                  if (retryTimes == 0 && domainEvent != null) _errorStore.SaveEventError(domainEvent.AggregateRootStringId, ex.ToString(), domainEvent.Version, message.GetActivity()?.TraceId.ToString() ?? message.Id);
                  throw;
              }
              finally
              {
                  message.GetActivity()?.Stop();
              }
          },
