    ...
    public class DefaultMemoryCache : IMemoryCache
    {
        ...
        public async Task<IAggregateRoot> GetAsync(object aggregateRootId, Type aggregateRootType)
        {
            ...
            if (_aggregateRootInfoDict.TryGetValue(aggregateRootId.ToString(), out AggregateCacheInfo aggregateRootInfo))
            {
                ...
                if (aggregateRoot.GetType() != aggregateRootType && aggregateRoot.GetType().BaseType != aggregateRootType)
                {
        ...
        public async Task<IAggregateRoot> RefreshAggregateFromEventStoreAsync(Type aggregateRootType, string aggregateRootId)
        {
            ...
            try
            {
                aggregateRoot = await _aggregateStorage.GetAsync(aggregateRootType, aggregateRootId.ToString()).ConfigureAwait(false);
                ResetAggregateRootCache(aggregateRootType, aggregateRootId, aggregateRoot);
                return aggregateRoot;
            }
            catch (Exception ex)
            {
                _logger.Error(string.Format("Refresh aggregate from event store has unknown exception, aggregateRootTypeName:{0}, aggregateRootId:{1}, version:{2}", _typeNameProvider.GetTypeName(aggregateRootType), aggregateRootId, aggregateRoot?.Version), ex);
                return null;
            }
        }
        public void Start()
        ...
        private void CleanInactiveAggregateRoot()
        {
            ...
            _logger.InfoFormat("inactive list count:{0}", inactiveList.Count);
