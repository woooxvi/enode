...
namespace ENode.Domain
{
    ...
    public abstract class AggregateRoot<TAggregateRootId> : IAggregateRoot
    {
        ...
        private void VerifyEvent(DomainEventStream eventStream)
        {
            ...
            {
                throw new InvalidOperationException(string.Format("Invalid domain event stream, aggregateRootId:{0}, expected aggregateRootId:{1}, type:{2},Version:{3}", eventStream.AggregateRootId, current.UniqueId, current.GetType().FullName, eventStream.Version));
            }
            if (eventStream.Version != current.Version + 1)
            {
                throw new InvalidOperationException(string.Format("Invalid domain event stream,eventStream version:{0}, current version:{1}, current aggregateRoot type:{2}, id:{3}", eventStream.Version, current.Version, this.GetType().FullName, current.UniqueId));
            }
        }
        ...
        protected Activity getTrace(string spanName)
        {
            if (spanName.IsNotEmpty())
            {
                if (ObjectContainer.TryResolve(out ActivitySource _activitySource))
                {
                    return _activitySource.StartActivity(spanName);
                }
            }
            return null;
        }
