    ...
    public class ProcessingEventMailBox
    {
        ...
        public EnqueueMessageResult EnqueueMessage(ProcessingEvent processingEvent)
        {
            lock (_lockObj)
            {
                ...
                if (_nextExpectingEventVersion == null || processingEvent.Message.Version > _nextExpectingEventVersion)
                {
        ...
        private void TryRemovedInvalidWaitingMessages(int nextExpectingEventVersion)
        {
            ...
            foreach (var key in toRemoveKeyList)
            {
                ...
                {
                    var act = processingEvent.Message.NextActivity("TryRemovedInvalidWaitingMessages");
                    act?.SetTag("version", processingEvent.Message.Version);
                    act?.SetTag("nextExpectingEventVersion", nextExpectingEventVersion);
                    ...
                    processingEvent.ProcessContext.NotifyEventProcessed();  //结束
                    act?.Stop();
                }
            }
        }
        ...
        private void ProcessMessage()
        {
            ...
            {
                var act = message.Message.NextActivity("ProcessMessage");
                LastActiveTime = DateTime.Now;
                try
                {
        ...
        private void EnqueueEventStream(ProcessingEvent processingEvent)
        {
            lock (_lockObj)
            {
                var act = processingEvent.Message.NextActivity("EnqueueEventStream");
