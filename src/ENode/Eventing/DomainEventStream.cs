...
namespace ENode.Eventing
{
    ...
    {
        public DomainEventStream(string commandId, string aggregateRootId, string aggregateRootTypeName, DateTime timestamp, int version, IEnumerable<IDomainEvent> events, IDictionary<string, string> items = null) : base()
        {
            if (events == null)
            {
            ...
            Version = (events.FirstOrDefault()?.Version) ?? version; //改
            Timestamp = timestamp;
            Events = events;
